...
public class PhotonGameAI : MonoBehaviourPun
{
        ...
        if (PhotonNetwork.IsMasterClient)
        {
            ...
            if (AI_Plane_Index != null)
            {
                for (int i = 0; i < AI_Plane_Index.Length; i++)
                {
                    ...
                    AI_Plane.GetComponent<AIProperty>().Initialize(string.Format("机器人{0}", i + 1));
                    photonView.RPC("AddAIPlaneList", RpcTarget.All, AI_Plane.name);
                }
            }
        }
    }
    ...
    [PunRPC]
    public void AddAIPlaneList(string name)
    {
